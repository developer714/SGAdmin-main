...
function install_sd_admin {
    ...
    pm2 start omb.config.js --env production
    pm2 save
...
function generate_cert {
    ...
    # Create a certificate for $LBBackendDomain
    cert_dir=/etc/nginx/sites-available/$LBBackendDomain/certs
    if [ ! -d $cert_dir ]
    then
    ...
    IFS="."; read -ra strarr <<< "$LBBackendDomain"
    IFS=""
    for (( n = 0; n < ${#strarr[*]}; n++))
    do
        if (( 0 < n )); then
    ...
    username=$(whoami)
    sudo chown -R $username /etc/nginx/sites-available/$LBBackendDomain/

    # Create a certificate for $BackendDomain
...
function config_nginx {
    # generate certificates for $LBBackendDomain
    cat <<EOF > ~/$LBBackendDomain.conf
server {
    listen 80;
    listen 443 ssl http2;
    server_name $LBBackendDomain;
    root /usr/share/nginx/html/;
    index index.html index.htm;
    ssl_certificate /etc/nginx/sites-available/$LBBackendDomain/certs/fullchain.pem;
    ssl_certificate_key /etc/nginx/sites-available/$LBBackendDomain/certs/privkey.pem;
...
EOF
    sudo mv -f ~/$LBBackendDomain.conf /etc/nginx/sites-available/$LBBackendDomain.conf
    username=$(whoami)
    sudo chown $username /etc/nginx/sites-available/$LBBackendDomain.conf

    if [ -f /etc/nginx/sites-enabled/$LBBackendDomain.conf ]
    then
        sudo unlink /etc/nginx/sites-enabled/$LBBackendDomain.conf
    fi
    sudo ln -s /etc/nginx/sites-available/$LBBackendDomain.conf /etc/nginx/sites-enabled/$LBBackendDomain.conf

    # generate certificates for $BackendDomain
