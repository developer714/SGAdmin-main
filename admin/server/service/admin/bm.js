...
// const { getBmLicenseStatus } = require("../config/bot");
...
/*
async function getBmLicenseStatus4Orgs(from, size) {
    const lmt = getMongooseLimitParam(from, size);
    const option = { bmpackage: { $nin: [undefined, null] } };
    ...
    const data = await Promise.all(
        orgs.map(async (org) => {
            const ret = await getBmLicenseStatus(org, false);
            ret.id = org.id;
            ret.title = org.title;
            return ret;
        })
...
async function __applyAwsS3Cfg() {
  const bm_engines = await getAllActiveBmEngineNodes();
  ...
  await Promise.all(
    bm_engines.map(async (bm_engine) => {
      try {
        await post2WafNodeApi(bm_engine, url, payload, jwtToken, false);
      } catch (err) {
