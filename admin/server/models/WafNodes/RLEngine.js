...
RLEngineSchema.virtual("isDeleted").get(function () {
  return !!this.deleted_at;
});

RLEngineSchema.virtual("addr").get(async function () {
...
RLEngineSchema.virtual("isActive").get(function () {
  if (!this.last_ping_at) return false;
  return this.last_ping_at.getTime() + 3 * CHECK_WAFS_HEALTH_PERIOD > Date.now();
});

const RLEngineModel = mongoose.model("wafedge", RLEngineSchema);

module.exports = { RLEngineModel, RLEngineSchema };
