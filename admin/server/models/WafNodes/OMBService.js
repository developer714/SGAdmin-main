...
OMBServiceSchema.virtual("isDeleted").get(function () {
  return !!this.deleted_at;
});

OMBServiceSchema.virtual("addr").get(async function () {
...
OMBServiceSchema.virtual("isActive").get(function () {
  if (!this.last_ping_at) return false;
  return this.last_ping_at.getTime() + 3 * CHECK_WAFS_HEALTH_PERIOD > Date.now();
});

const OMBServiceModel = mongoose.model("ombservicenode", OMBServiceSchema);

module.exports = { OMBServiceModel, OMBServiceSchema };
