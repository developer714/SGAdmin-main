...
BMEngineSchema.virtual("isDeleted").get(function () {
  return !!this.deleted_at;
});

BMEngineSchema.virtual("addr").get(async function () {
...
BMEngineSchema.virtual("isActive").get(function () {
  if (!this.last_ping_at) return false;
  return this.last_ping_at.getTime() + 3 * CHECK_WAFS_HEALTH_PERIOD > Date.now();
});

const BMEngineModel = mongoose.model("bmenginenode", BMEngineSchema);

module.exports = { BMEngineModel, BMEngineSchema };
