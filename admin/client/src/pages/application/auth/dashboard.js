...
import Data from "../../../components/pages/application/auth/L_Data";
import AuthFilterResultModal from "../../../components/pages/application/auth/M_AuthFilter";
import AuthScoreStatsChart from "../../../components/pages/application/auth/C_AuthScoreStats";
...
import AuthScoreTotalChart from "../../../components/pages/application/auth/C_AuthScoreTotal";
...
function AUDashboard() {
  ...
  const { getOnlyAuthEvents } = useEvent();
  const {
    top_source,
    top_path,
    top_ua,
  ...
  React.useEffect(() => {
    if (filter[0].key && filter[0].values) {
      if (siteID) {
        getOnlyAuthEvents(siteID, getTimeRange(), rows_per_page, 0, WafAction.ALL, filter);
      }
      setFilterOpen(true);
    }
  }, [filter, siteID, getOnlyAuthEvents]); // eslint-disable-line react-hooks/exhaustive-deps
  React.useEffect(() => {
  ...
  React.useEffect(() => {
    ...
    if (planController) planController.abort();
    if (siteID) {
      getAuthScoreStatsTotal(setWafDashController, siteID, getTimeRange());
    }
  ...
  const getAuthScoreStatsWithFilter = useCallback(() => {
    if (homeController) homeController.abort();
    if (wafdashController) wafdashController.abort();
    if (websiteController) websiteController.abort();
    if (wafeventController) wafeventController.abort();
    if (planController) planController.abort();
    if (siteID) {
      getAuthScoreStats(setWafEventController, siteID, getTimeRange());
    }
  }, [
    homeController,
    wafdashController,
  ...
  const selectCurrentSite = (event) => {
    navigate(`/application/${event.target.value}/auth/dashboard`);
  };
  ...
  return (
    <React.Fragment>
      <Helmet title="Auth Management - Dashboard" />

      <Grid container spacing={6} mt={"14px"}>
        <Grid item>
          <Typography variant="h1" gutterBottom display="inline">
            Auth Management Dashboard
          </Typography>
        </Grid>
        <Grid item xs></Grid>
      ...
      <AuthFilterResultModal open={filterOpen} handleClose={filterClose} siteID={siteID} timeRange={getTimeRange()} />
