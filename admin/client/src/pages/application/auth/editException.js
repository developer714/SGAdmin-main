...
function EditAuthException() {
  const navigate = useNavigate();
  const { configSite, authExceptionID } = useParams();
  const { isAuthenticated, userRole } = useAuth();
  const { getCurrentAuthException, updateAuthException, setErr, errMsg } = useAUConfig();
  ...
  React.useEffect(() => {
    ...
    const init = async () => {
      setInitFlag(true);
      setErr(null);
      const res = await getCurrentAuthException(siteUid, authExceptionID);
      setAuthExceptionName(res?.name ? res?.name : undefined);
      setConditions(
    ...
    return () => setErr(null);
  }, [isAuthenticated, getCurrentAuthException, setErr, siteUid, authExceptionID]);
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    const result = await updateAuthException(siteUid, authExceptionID, {
  ...
  return (
    <React.Fragment>
      <Helmet title={UserRole.READONLY_USER === userRole ? "View Auth Exception" : "Edit Auth Exception"} />
      <Grid container sx={{ display: "flex", alignItems: "center" }} pt={9} pb={6}>
        <Grid item>
          <Typography variant="h1" display="inline">
            {UserRole.READONLY_USER === userRole ? "View Auth Exception" : "Edit Auth Exception"}
          </Typography>
        </Grid>
        <Grid item xs></Grid>
      </Grid>
...
export default EditAuthException;
