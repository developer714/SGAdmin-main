...
import AuthEventTable from "../../../components/pages/application/analytics/T_AuthEvent";
import AuthEventStatsChart from "../../../components/pages/application/analytics/C_AuthEventStats";
...
function AnalyticsAuthEvents() {
  ...
  const { top_source, top_path, top_method, top_response_code, top_auth_score, getTopAuthEventsInfo } = useAUConfig();
  const { site_id, time_range, getAuthEvents, getAuthEventStats, rows_per_page, errMsg, setErr } = useEvent();
  ...
  const getEventsWithFillter = (collectStats = true) => {
    ...
    if (collectStats) {
      ...
      getTopAuthEventsInfo(setWafEventController, siteID, getTimeRange());
    }
    getAuthEvents(setWafEventController, siteID, getTimeRange(), rows_per_page, 0, wafAction, _conditions);
    getAuthEventStats(setWafEventController, siteID, getTimeRange(), _conditions);
  };
  //////////////////////////////////////////////
      ...
      {showFilter ? (
        <Box sx={{ background: "white", padding: "32px 12px", borderRadius: "8px 0px 8px 8px" }}>
          <Grid container spacing={3}>
            ...
            <Grid item xs={2.4}>
              <Typography variant="h3">AUTH SCORE</Typography>
            </Grid>
            ...
            <Grid item xs={2.4} sx={{ borderRight: "solid 1px #C1C1C1" }}>
              <CheckKeyAllComponent
                type="auth_score"
                isChecked={conditions.find((condition) => condition.key === "auth_score")?.values?.length === top_auth_score?.length}
                values={top_auth_score?.map((entry) => entry.auth_score)}
                handleFilterAllChange={handleFilterAllChange}
              />
              {null === top_auth_score ? (
                <Loader />
              ) : (
                top_auth_score.map((entry) => (
                  <CheckKeyValueComponent
                    type="auth_score"
                    keyStr={entry.auth_score}
                    valueNumber={entry.count}
                    handleFilterChange={handleFilterChange}
                    isChecked={
                      !!conditions.find((condition) => condition.key === "auth_score")?.values?.find((value) => value === entry.auth_score)
                    }
                  />
                ))
              )}
        ...
        <>
          ...
          <Grid container spacing={6}>
            <Grid item xs={12}>
              ...
              <AuthEventTable pattern={pattern} />
            </Grid>
          </Grid>
        </>
      )}
...
export default AnalyticsAuthEvents;
