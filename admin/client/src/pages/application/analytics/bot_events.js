...
import BotEventTable from "../../../components/pages/application/analytics/T_BotEvent";
import BotEventStatsChart from "../../../components/pages/application/analytics/C_BotEventStats";
...
function AnalyticsBotEvents() {
  ...
  const { top_source, top_path, top_method, top_response_code, top_bot_score, getTopBotEventsInfo } = useBMConfig();
  const { site_id, time_range, getBotEvents, getBotEventStats, rows_per_page, errMsg, setErr } = useEvent();
  ...
  const getEventsWithFillter = (collectStats = true) => {
    ...
    if (collectStats) {
      ...
      getTopBotEventsInfo(setWafEventController, siteID, getTimeRange());
    }
    getBotEvents(setWafEventController, siteID, getTimeRange(), rows_per_page, 0, wafAction, _conditions);
    getBotEventStats(setWafEventController, siteID, getTimeRange(), _conditions);
  };
  //////////////////////////////////////////////
      ...
      {showFilter ? (
        <Box sx={{ background: "white", padding: "32px 12px", borderRadius: "8px 0px 8px 8px" }}>
          <Grid container spacing={3}>
            ...
            <Grid item xs={2.4}>
              <Typography variant="h3">BOT SCORE</Typography>
            </Grid>
            ...
            <Grid item xs={2.4} sx={{ borderRight: "solid 1px #C1C1C1" }}>
              <CheckKeyAllComponent
                type="bot_score"
                isChecked={conditions.find((condition) => condition.key === "bot_score")?.values?.length === top_bot_score?.length}
                values={top_bot_score?.map((entry) => entry.bot_score)}
                handleFilterAllChange={handleFilterAllChange}
              />
              {null === top_bot_score ? (
                <Loader />
              ) : (
                top_bot_score.map((entry) => (
                  <CheckKeyValueComponent
                    type="bot_score"
                    keyStr={entry.bot_score}
                    valueNumber={entry.count}
                    handleFilterChange={handleFilterChange}
                    isChecked={
                      !!conditions.find((condition) => condition.key === "bot_score")?.values?.find((value) => value === entry.bot_score)
                    }
                  />
                ))
              )}
