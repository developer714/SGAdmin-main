...
function EditRateLimit() {
  const navigate = useNavigate();
  const { configSite, ratelimitID } = useParams();
  const { isAuthenticated, userRole, isFeatureEnabled } = useAuth();
  const { getCurrentRateLimit, updateRateLimit, setErr, errMsg } = useRateLimit();
  ...
  React.useEffect(() => {
    ...
    const init = async () => {
      setInitFlag(true);
      setErr(null);
      const res = await getCurrentRateLimit(siteUid, ratelimitID);
      setRateLimitName(res?.name ? res?.name : undefined);
      setConditions(
        res?.conditions && 0 < res.conditions.length && 0 < res.conditions[0].length
          ? res.conditions
          : [[{ key: "none", condition: "none", value: "" }]]
      );
      setCharacteristics(res?.characteristics || [{ key: "none", value: "" }]);
      setAction(res?.action ? res?.action : FirewallAction.BLOCK);
      setRequestsPerPeriod(res?.requests_per_period ? res.requests_per_period : 1);
      setPeriod(res?.period ? res.period : RateLimitPeriod.TEN_SECONDS);
      setMitigationTimeout(res?.mitigation_timeout ? res.mitigation_timeout : RateLimitMitigationTimeout.THIRTY_SECONDS);
      setInitFlag(false);
    };
    if (isAuthenticated) init();
    return () => setErr(null);
  }, [isAuthenticated, getCurrentRateLimit, setErr, siteUid, ratelimitID]);
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    const result = await updateRateLimit(siteUid, ratelimitID, {
      action,
      name: ratelimitName,
  ...
  return (
    <React.Fragment>
      <Helmet title={UserRole.READONLY_USER === userRole ? "View Rate Limiting Rule" : "Edit Rate Limiting Rule"} />
      <Grid container sx={{ display: "flex", alignItems: "center" }} pt={9} pb={6}>
        <Grid item>
          <Typography variant="h1" display="inline">
            {UserRole.READONLY_USER === userRole ? "View Rate Limiting Rule" : "Edit Rate Limiting Rule"}
          </Typography>
        </Grid>
        <Grid item xs></Grid>
      </Grid>
              ...
              <Grid container display="flex" alignItems="center">
                <Grid item xs={12} md={3}>
                  <TextField
                    ...
                    sx={{ width: "95%", marginBottom: "8px", background: "white" }}
                  />
                </Grid>
                <Grid item xs={12} md={3}>
