...
function EnhancedTable() {
  const { otxApis, apiTotal, apiFrom, apiSize, getOtxApiKeyHistory } = useApiKey();
  ...
  const handleChangePage = async (event, newPage) => {
    setPage(newPage);
    getOtxApiKeyHistory(rowsPerPage, rowsPerPage * newPage, false);
  };

  const handleChangeRowsPerPage = async (event) => {
    setPage(0);
    setRowsPerPage(parseInt(event.target.value, 10));
    getOtxApiKeyHistory(parseInt(event.target.value, 10), 0, false);
  };
        ...
        <TableContainer>
          <Table aria-labelledby="tableTitle" size={"medium"} aria-label="enhanced table">
            ...
            {otxApis === null ? (
              ...
              <TableBody>
                {stableSort(otxApis, getComparator(order, orderBy)).map((row, index) => {
                  return (
                    <TableRow>
                      <TableCell
                        align="left"
