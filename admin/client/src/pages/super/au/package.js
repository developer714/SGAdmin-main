...
import DeleteAuPackageModal from "../../../components/pages/application/auth/M_DeleteAuPackage";

function SAAUPackage() {
  const { getAuPackage, getOrganisations, createAuPackage, errMsg, setErr } = useAU();
  ...
  const getAu = useCallback(async () => {
    ...
    const result = await getAuPackage(curOrg);
    if (result) {
      setNewPackage(false);
      ...
      setNewPackage(true);
      setMessage("No Auth management package for this organisation");
      setSuccess("info");
      setSnackOpen(true);
      setCurPlanPrice(0);
      setCurPlanPeriod(1);
  ...
  React.useEffect(() => {
    ...
    getAu();
  }, [curOrg, getAuPackage, setErr, getAu]);
  const refresh = () => {
    getAu();
  };
  ...
  const save = async () => {
    setSaving(true);
    const result = await createAuPackage(
      newPackage,
      curOrg,
      numberOfSite,
      pricePerSite,
  ...
  return (
    <React.Fragment>
      <Helmet title="SA Auth Management Package Management" />
      <Grid container sx={{ display: "flex", alignItems: "center" }}>
        <Grid item>
          <Typography variant="h3" gutterBottom display="inline">
            Auth Management Package Management
          </Typography>
        </Grid>
        <Grid item xs></Grid>
