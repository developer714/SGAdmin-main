...
function EnhancedTable() {
  const { apiHistory, apiTotal, apiFrom, apiSize, getStripeApiKeyHistory } = usePayment();
  ...
  const handleChangePage = async (event, newPage) => {
    setPage(newPage);
    getStripeApiKeyHistory(rowsPerPage, rowsPerPage * newPage, false);
  };

  const handleChangeRowsPerPage = async (event) => {
    setPage(0);
    setRowsPerPage(parseInt(event.target.value, 10));
    getStripeApiKeyHistory(parseInt(event.target.value, 10), 0, false);
  };
        ...
        <TableContainer>
          <Table aria-labelledby="tableTitle" size={"medium"} aria-label="enhanced table">
            ...
            {apiHistory === null ? (
              ...
              <TableBody>
                {stableSort(apiHistory, getComparator(order, orderBy)).map((row, index) => {
                  return (
                    <TableRow>
                      <TableCell
                        align="left"
...
function StripeApiKeyHistoryList() {
  return <EnhancedTable />;
}
export default StripeApiKeyHistoryList;
