...
const headCells = [
  ...
  },
  {
    id: "unit_requests",
    alignment: "left",
    label: "Billable Unit Requests",
  },
  {
    id: "unit_price",
    alignment: "left",
    label: "Billable Unit Price ($)",
  },
  {
    id: "updated",
    alignment: "left",
...
function EnhancedTable() {
  const { rateLimitBillHistory, rateLimitBillTotal, rateLimitBillFrom, rateLimitBillSize, getRateLimitBillHistory } = usePayment();
  var totalCount = rateLimitBillTotal;
  ...
  React.useEffect(() => {
    setPage(rateLimitBillFrom / rateLimitBillSize);
  }, [rateLimitBillFrom, rateLimitBillSize]);
  React.useEffect(() => {
    setRowsPerPage(rateLimitBillSize);
  }, [rateLimitBillSize]);
  ...
  const handleChangePage = async (event, newPage) => {
    setPage(newPage);
    getRateLimitBillHistory(rowsPerPage, rowsPerPage * newPage, false);
  };

  const handleChangeRowsPerPage = async (event) => {
    setPage(0);
    setRowsPerPage(parseInt(event.target.value, 10));
    getRateLimitBillHistory(parseInt(event.target.value, 10), 0, false);
  };
        ...
        <TableContainer>
          <Table aria-labelledby="tableTitle" size={"medium"} aria-label="enhanced table">
            ...
            {rateLimitBillHistory === null ? (
              ...
              <TableBody>
                {stableSort(rateLimitBillHistory, getComparator(order, orderBy)).map((row, index) => {
                  return (
                    <TableRow>
                      <TableCell
                        align="left"
...
function RateLimitBillHistoryList() {
  return <EnhancedTable />;
}
export default RateLimitBillHistoryList;
