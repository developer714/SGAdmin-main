...
function EnhancedTable({ curOrg }) {
  const { bmHistory, bmTotal, bmFrom, bmSize, getBmPaymentHistory } = useBM();
  var totalCount = bmTotal;
  ...
  React.useEffect(() => {
    setPage(bmFrom / bmSize);
  }, [bmFrom, bmSize]);
  React.useEffect(() => {
    setRowsPerPage(bmSize);
  }, [bmSize]);
  ...
  const handleChangePage = async (event, newPage) => {
    setPage(newPage);
    getBmPaymentHistory(curOrg, rowsPerPage, rowsPerPage * newPage, false);
  };

  const handleChangeRowsPerPage = async (event) => {
    setPage(0);
    setRowsPerPage(parseInt(event.target.value, 10));
    getBmPaymentHistory(curOrg, parseInt(event.target.value, 10), 0, false);
  };
        ...
        <TableContainer>
          <Table aria-labelledby="tableTitle" size={"medium"} aria-label="enhanced table">
            ...
            {bmHistory === null ? (
              ...
              <TableBody>
                {stableSort(bmHistory, getComparator(order, orderBy)).map((row, index) => {
                  return (
                    <TableRow>
                      <TableCell
                        align="left"
...
function BmPaymentHistoryList({ curOrg }) {
  return <EnhancedTable curOrg={curOrg} />;
}
export default BmPaymentHistoryList;
