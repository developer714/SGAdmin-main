...
function EnhancedTable({ curOrg }) {
  const { cusHistory, cusTotal, cusFrom, cusSize, getCustomPaymentHistory } = usePayment();
  var totalCount = cusTotal;
  ...
  React.useEffect(() => {
    setPage(cusFrom / cusSize);
  }, [cusFrom, cusSize]);
  React.useEffect(() => {
    setRowsPerPage(cusSize);
  }, [cusSize]);
  ...
  const handleChangePage = async (event, newPage) => {
    setPage(newPage);
    getCustomPaymentHistory(curOrg, rowsPerPage, rowsPerPage * newPage, false);
  };

  const handleChangeRowsPerPage = async (event) => {
    setPage(0);
    setRowsPerPage(parseInt(event.target.value, 10));
    getCustomPaymentHistory(curOrg, parseInt(event.target.value, 10), 0, false);
  };
        ...
        <TableContainer>
          <Table aria-labelledby="tableTitle" size={"medium"} aria-label="enhanced table">
            ...
            {cusHistory === null ? (
              ...
              <TableBody>
                {stableSort(cusHistory, getComparator(order, orderBy)).map((row, index) => {
                  return (
                    <TableRow>
                      <TableCell
                        align="left"
...
function CustomPaymentHistoryList({ curOrg }) {
  return <EnhancedTable curOrg={curOrg} />;
}
export default CustomPaymentHistoryList;
