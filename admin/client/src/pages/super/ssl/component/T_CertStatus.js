...
function EnhancedTable() {
  const { certStatus, certTotal, certFrom, certSize, getCertificateHistory } = useSSL();
  var totalCount = certTotal;
  ...
  React.useEffect(() => {
    setPage(certFrom / certSize);
  }, [certFrom]); // eslint-disable-line react-hooks/exhaustive-deps
  React.useEffect(() => {
    setRowsPerPage(certSize);
  }, [certSize]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleChangePage = async (event, newPage) => {
    setPage(newPage);
    getCertificateHistory(rowsPerPage, rowsPerPage * newPage, false);
  };

  const handleChangeRowsPerPage = async (event) => {
    setPage(0);
    setRowsPerPage(parseInt(event.target.value, 10));
    getCertificateHistory(parseInt(event.target.value, 10), 0, false);
  };
          ...
          <Table aria-labelledby="tableTitle" size={"medium"} aria-label="enhanced table">
            ...
            {certStatus === null ? (
              ...
              <TableBody>
                <TableRow>
                  <TableCell colSpan={9}>
                    <Typography textAlign="center">There are no certificates</Typography>
                  </TableCell>
                </TableRow>
              </TableBody>
            ) : (
              <TableBody>
                {stableSort(certStatus, getComparator(order, orderBy)).map((row, index) => {
                  return (
                    <TableRow>
                      <TableCell
                        align="left"
...
function CertStatusList() {
  return <EnhancedTable />;
}
export default CertStatusList;
