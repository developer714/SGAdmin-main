...
function EnhancedTable() {
  const { zcrmApiConfigHistory, zcrmApiConfigTotal, zcrmApiConfigFrom, zcrmApiConfigSize, getZohoCrmApiConfigHistory } = useZcrm();
  ...
  React.useEffect(() => {
    setPage(zcrmApiConfigFrom / zcrmApiConfigSize);
  }, [zcrmApiConfigFrom, zcrmApiConfigSize]);
  React.useEffect(() => {
    setRowsPerPage(zcrmApiConfigSize);
  }, [zcrmApiConfigSize]);
  ...
  const handleChangePage = async (event, newPage) => {
    setPage(newPage);
    getZohoCrmApiConfigHistory(rowsPerPage, rowsPerPage * newPage, false);
  };

  const handleChangeRowsPerPage = async (event) => {
    setPage(0);
    setRowsPerPage(parseInt(event.target.value, 10));
    getZohoCrmApiConfigHistory(parseInt(event.target.value, 10), 0, false);
  };
        ...
        <TableContainer>
          <Table aria-labelledby="tableTitle" size={"medium"} aria-label="enhanced table">
            ...
            {zcrmApiConfigHistory === null ? (
              ...
              <TableBody>
                {stableSort(zcrmApiConfigHistory, getComparator(order, orderBy)).map((row, index) => {
                  return (
                    <TableRow>
                      <TableCell
                        align="left"
...
function ZohoCrmApiConfigHistoryList() {
  return <EnhancedTable />;
}
export default ZohoCrmApiConfigHistoryList;
