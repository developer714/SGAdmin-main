...
function EnhancedTable() {
  const { hCaptchaSiteKeys, hCaptchaSiteKeyTotal, hCaptchaSiteKeyFrom, hCaptchaSiteKeySize, getHcaptchaSiteKeyHistory } = useCaptcha();
  var totalCount = hCaptchaSiteKeyTotal;
  ...
  React.useEffect(() => {
    setPage(hCaptchaSiteKeyFrom / hCaptchaSiteKeySize);
  }, [hCaptchaSiteKeyFrom, hCaptchaSiteKeySize]);
  React.useEffect(() => {
    setRowsPerPage(hCaptchaSiteKeySize);
  }, [hCaptchaSiteKeySize]);
  ...
  const handleChangePage = async (event, newPage) => {
    setPage(newPage);
    getHcaptchaSiteKeyHistory(rowsPerPage, rowsPerPage * newPage, false);
  };

  const handleChangeRowsPerPage = async (event) => {
    setPage(0);
    setRowsPerPage(parseInt(event.target.value, 10));
    getHcaptchaSiteKeyHistory(parseInt(event.target.value, 10), 0, false);
  };
        ...
        <TableContainer>
          <Table aria-labelledby="tableTitle" size={"medium"} aria-label="enhanced table">
            ...
            {hCaptchaSiteKeys === null ? (
              ...
              <TableBody>
                {stableSort(hCaptchaSiteKeys, getComparator(order, orderBy)).map((row, index) => {
                  return (
                    <TableRow>
                      <TableCell
                        align="left"
...
function HcaptchaSiteKeyHistoryList() {
  return <EnhancedTable />;
}
export default HcaptchaSiteKeyHistoryList;
