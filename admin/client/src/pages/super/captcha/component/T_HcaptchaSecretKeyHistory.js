...
function EnhancedTable() {
  const { hCaptchaSecretKeys, hCaptchaSecretKeyTotal, hCaptchaSecretKeyFrom, hCaptchaSecretKeySize, getHcaptchaSecretKeyHistory } =
    useCaptcha();
  var totalCount = hCaptchaSecretKeyTotal;
  ...
  React.useEffect(() => {
    setPage(hCaptchaSecretKeyFrom / hCaptchaSecretKeySize);
  }, [hCaptchaSecretKeyFrom, hCaptchaSecretKeySize]);
  React.useEffect(() => {
    setRowsPerPage(hCaptchaSecretKeySize);
  ...
  const handleChangePage = async (event, newPage) => {
    setPage(newPage);
    getHcaptchaSecretKeyHistory(rowsPerPage, rowsPerPage * newPage, false);
  };

  const handleChangeRowsPerPage = async (event) => {
    setPage(0);
    setRowsPerPage(parseInt(event.target.value, 10));
    getHcaptchaSecretKeyHistory(parseInt(event.target.value, 10), 0, false);
  };
        ...
        <TableContainer>
          <Table aria-labelledby="tableTitle" size={"medium"} aria-label="enhanced table">
            ...
            {hCaptchaSecretKeys === null ? (
              ...
              <TableBody>
                {stableSort(hCaptchaSecretKeys, getComparator(order, orderBy)).map((row, index) => {
                  return (
                    <TableRow>
                      <TableCell
                        align="left"
...
function HcaptchaSecretKeyHistoryList() {
  return <EnhancedTable />;
}
export default HcaptchaSecretKeyHistoryList;
