...
function EnhancedTable() {
  const { deleteUsers, deleteTotal, time_range, deleteFrom, deleteSize, getDeleteUserReport } = useUser();
  var totalCount = deleteTotal;
  const [order, setOrder] = React.useState("desc");
  const [orderBy, setOrderBy] = React.useState("deleted");
  const [page, setPage] = React.useState(0);
  const [rowsPerPage, setRowsPerPage] = React.useState(5);

  React.useEffect(() => {
    setPage(deleteFrom / deleteSize);
  }, [deleteFrom]); // eslint-disable-line react-hooks/exhaustive-deps
  React.useEffect(() => {
    setRowsPerPage(deleteSize);
  }, [deleteSize]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleChangePage = async (event, newPage) => {
    setPage(newPage);
    getDeleteUserReport(time_range, rowsPerPage, rowsPerPage * newPage);
  };

  const handleChangeRowsPerPage = async (event) => {
    setPage(0);
    setRowsPerPage(parseInt(event.target.value, 10));
    getDeleteUserReport(time_range, parseInt(event.target.value, 10), 0);
  };
  return (
    <>
      <Paper>
          ...
          <Table aria-labelledby="tableTitle" size={"medium"} aria-label="enhanced table">
            ...
            {deleteUsers === null ? (
              ...
              <TableBody>
                <TableRow>
                  <TableCell colSpan={9}>
                    <Typography textAlign="center">There are no deleted users</Typography>
                  </TableCell>
                </TableRow>
              </TableBody>
            ) : (
              <TableBody>
                {stableSort(deleteUsers, getComparator(order, orderBy)).map((row, index) => {
                  return (
