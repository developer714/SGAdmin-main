...
function EnhancedTable() {
  const { authInfos, authInfoTotal, authInfoFrom, authInfoSize, getAuthInfoHistory } = useElastic();
  var totalCount = authInfoTotal;
  ...
  React.useEffect(() => {
    setPage(authInfoFrom / authInfoSize);
  }, [authInfoFrom]); // eslint-disable-line react-hooks/exhaustive-deps
  React.useEffect(() => {
    setRowsPerPage(authInfoSize);
  }, [authInfoSize]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleChangePage = async (event, newPage) => {
    setPage(newPage);
    getAuthInfoHistory(rowsPerPage, rowsPerPage * newPage, false);
  };

  const handleChangeRowsPerPage = async (event) => {
    setPage(0);
    setRowsPerPage(parseInt(event.target.value, 10));
    getAuthInfoHistory(parseInt(event.target.value, 10), 0, false);
  };

  return (
    <>
      <Paper sx={{ width: { xs: "100%", md: "90%" } }}>
        <TableContainer>
          <Table aria-labelledby="tableTitle" size={"medium"} aria-label="enhanced table">
            ...
            {authInfos === null ? (
              ...
              <TableBody>
                {stableSort(authInfos, getComparator(order, orderBy)).map((row, index) => {
                  return (
                    <TableRow>
                      <TableCell
                        align="left"
...
function AuthInfoHistoryList() {
  return <EnhancedTable />;
}
export default AuthInfoHistoryList;
