...
function EnhancedTable() {
  const { apis, apiTotal, apiFrom, apiSize, getApiKeyHistory } = useElastic();
  ...
  }, [apiFrom]); // eslint-disable-line react-hooks/exhaustive-deps
  React.useEffect(() => {
    setRowsPerPage(apiSize);
  }, [apiSize]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleChangePage = async (event, newPage) => {
    setPage(newPage);
    getApiKeyHistory(rowsPerPage, rowsPerPage * newPage, false);
  };

  const handleChangeRowsPerPage = async (event) => {
    setPage(0);
    setRowsPerPage(parseInt(event.target.value, 10));
    getApiKeyHistory(parseInt(event.target.value, 10), 0, false);
  };
        ...
        <TableContainer>
          <Table aria-labelledby="tableTitle" size={"medium"} aria-label="enhanced table">
            ...
            {apis === null ? (
              ...
              <TableBody>
                {stableSort(apis, getComparator(order, orderBy)).map((row, index) => {
                  return (
                    <TableRow>
                      <TableCell
                        align="left"
...
function ApiKeyHistoryList() {
  return <EnhancedTable />;
}
export default ApiKeyHistoryList;
