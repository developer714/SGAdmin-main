...
function EnhancedTable() {
  const { addresses, addressTotal, addressFrom, addressSize, getAddressHistory } = useElastic();
  var totalCount = addressTotal;
  ...
  React.useEffect(() => {
    setPage(addressFrom / addressSize);
  }, [addressFrom]); // eslint-disable-line react-hooks/exhaustive-deps
  React.useEffect(() => {
    setRowsPerPage(addressSize);
  }, [addressSize]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleChangePage = async (event, newPage) => {
    setPage(newPage);
    getAddressHistory(rowsPerPage, rowsPerPage * newPage, false);
  };

  const handleChangeRowsPerPage = async (event) => {
    setPage(0);
    setRowsPerPage(parseInt(event.target.value, 10));
    getAddressHistory(parseInt(event.target.value, 10), 0, false);
  };
        ...
        <TableContainer>
          <Table aria-labelledby="tableTitle" size={"medium"} aria-label="enhanced table">
            ...
            {addresses === null ? (
              ...
              <TableBody>
                {stableSort(addresses, getComparator(order, orderBy)).map((row, index) => {
                  return (
                    <TableRow>
                      <TableCell
                        align="left"
