...
function EnhancedTable() {
  const { licenseStatus4Orgs, total_organisation_count, from, rows_per_page, getLicenseStatus4Orgs } = useAU();
                ...
                {stableSort(licenseStatus4Orgs, getComparator(order, orderBy)).map((row, index) => {
                  return (
                    <TableRow hover role="checkbox" tabIndex={-1} key={`${row?.id}-${index}`}>
                      ...
                      >
                        {row?.au_expire_at && getAlertTypography(new Date(row?.au_expire_at) > new Date(), formatDate(row?.au_expire_at))}
                      </TableCell>
                    </TableRow>
                  );
                })}
