...
import { getAuthType } from "../analytics/common";
// import { AuthScore } from "../../../../utils/constants";
...
const EnhancedTableHead = (props) => {
  const headCells = [
    ...
    { id: "auth_score", alignment: "left", label: "Auth Type" },
    /*
        {            id: "uri",
            alignment: "left",
            label: "URI",
...
function EnhancedTable({ pattern }) {
  ...
  const { authEvents, site_id, time_range, total_auth_events_count, from, rows_per_page, action, conditions, getOnlyAuthEvents } = useEvent();

  var totalCount = total_auth_events_count;
  ...
  const handleChangePage = async (event, newPage) => {
    setPage(newPage);
    getOnlyAuthEvents(site_id, time_range, rowsPerPage, rowsPerPage * newPage, action, conditions);
  };
  ...
  const handleChangeRowsPerPage = async (event) => {
    ...
    getOnlyAuthEvents(site_id, time_range, parseInt(event.target.value, 10), 0, action, conditions);
  };

  const gotoDetail = (id) => {
    navigate("/application/analytics/auth_events/" + id);
  };
          ...
          <Table aria-labelledby="tableTitle" size={"medium"} aria-label="enhanced table">
            ...
            {authEvents === null ? (
              ...
              <TableBody>
                <TableRow>
                  <TableCell colSpan={9}>
                    <Typography textAlign="center">There are no auth events</Typography>
                  </TableCell>
                </TableRow>
              </TableBody>
            ) : (
              <TableBody>
                {stableSort(authEvents, getComparator(order, orderBy)).map((row, index) => {
                  var showFlag = false;
                  ...
                  if (getAuthType(row?.auth_score).toString().search(pattern) >= 0) showFlag = true;
                  ...
                  return (
                    <TableRow hover tabIndex={-1} key={`${row?.id}-${index}`} onClick={(event) => handleClick(event, row?.id)}>
                      ...
                      <TableCell align="left" sx={{ padding: "8px" }}>
                        <Chip
                          label={getAuthType(row?.auth_score)}
...
function AuthEventList({ pattern }) {
  return <EnhancedTable pattern={pattern} />;
}
export default AuthEventList;
