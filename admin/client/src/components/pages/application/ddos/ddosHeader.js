...
function DdosHeader({ title, onSiteChange, onRefreshClick, onlyShowSites, children }) {
  ...
  const apply = async () => {
    ...
    if (site) {
      const result = await settingApply(site.site_id, ConfigAction.DDOS);
