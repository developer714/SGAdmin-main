...
const BotEventStatsChart = ({ setCustomRangeConfirm, setTimeRange, setCustomDateRange }) => {
  const theme = useTheme();
  const { botEventStats } = useEvent();
  ...
  React.useEffect(() => {
    let categories = [];
    const wafEventDatas = [];
    if (botEventStats?.length > 0) {
      botEventStats.forEach((stat) => {
    ...
    if (categories.length > 0) {
      categories.forEach((cate) => {
        if (null === botEventStats || 0 === botEventStats.length) {
          wafEventDatas.push(0);
        } else {
          let sig = botEventStats.find((x) => x.key_as_string === cate);
          if (undefined === sig) {
            wafEventDatas.push(0);
          } else {
  ...
  }, [botEventStats, setTimeRange, setCustomDateRange, setCustomRangeConfirm, theme.palette.custom.yellow.opacity_80]);
...
export default withTheme(BotEventStatsChart);
