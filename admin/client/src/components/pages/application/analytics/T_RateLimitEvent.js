...
const EnhancedTableHead = (props) => {
  ...
  const headCells = [
    ...
    { id: "ja3_hash", alignment: "left", label: "JA3 Fingerprint" },
    /*
        {            id: "uri",
            alignment: "left",
            label: "URI",
...
function EnhancedTable({ pattern }) {
  ...
  const { rlEvents, site_id, time_range, total_rl_events_count, from, rows_per_page, action, conditions, getOnlyRlEvents } = useEvent();

  var totalCount = total_rl_events_count;
  ...
  const handleChangePage = async (event, newPage) => {
    setPage(newPage);
    getOnlyRlEvents(site_id, time_range, rowsPerPage, rowsPerPage * newPage, action, conditions);
  };
  ...
  const handleChangeRowsPerPage = async (event) => {
    ...
    getOnlyRlEvents(site_id, time_range, parseInt(event.target.value, 10), 0, action, conditions);
  };

  const gotoDetail = (id) => {
    navigate("/application/analytics/rl_events/" + id);
  };
          ...
          <Table aria-labelledby="tableTitle" size={"medium"} aria-label="enhanced table">
            ...
            {rlEvents === null ? (
              ...
              <TableBody>
                <TableRow>
                  <TableCell colSpan={9}>
                    <Typography textAlign="center">There are no rate limit events</Typography>
                  </TableCell>
                </TableRow>
              </TableBody>
            ) : (
              <TableBody>
                {stableSort(rlEvents, getComparator(order, orderBy)).map((row, index) => {
                  var showFlag = false;
...
function RateLimitEventList({ pattern }) {
  return <EnhancedTable pattern={pattern} />;
}
export default RateLimitEventList;
