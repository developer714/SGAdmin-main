...
const WafEventStatsChart = ({ setCustomRangeConfirm, setTimeRange, setCustomDateRange }) => {
  const theme = useTheme();
  const { wafEventStats } = useEvent();
  ...
  React.useEffect(() => {
    let categories = [];
    const wafEventDatas = [];
    if (wafEventStats?.length > 0) {
      wafEventStats.forEach((stat) => {
    ...
    if (categories.length > 0) {
      categories.forEach((cate) => {
        if (null === wafEventStats || 0 === wafEventStats.length) {
          wafEventDatas.push(0);
        } else {
          let sig = wafEventStats.find((x) => x.key_as_string === cate);
          if (undefined === sig) {
            wafEventDatas.push(0);
          } else {
            wafEventDatas.push(sig.doc_count);
          }
        }
      });
    }
  ...
  }, [wafEventStats, setTimeRange, setCustomDateRange, setCustomRangeConfirm, theme.palette.custom.yellow.opacity_80]);
      ...
      <CardContent sx={{ background: "white", borderRadius: "8px" }}>
        <ChartWrapper>
          {null === wafEventStats ? (
            ...
            <ReactApexChart color="success" options={options} series={data} type="bar" height="250" />
          )}
        </ChartWrapper>
      </CardContent>
    </Card>
...
export default withTheme(WafEventStatsChart);
