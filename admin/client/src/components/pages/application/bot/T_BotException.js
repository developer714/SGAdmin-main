...
import DeleteBotExceptionModal from "./M_DeleteBotException";
...
function EnhancedTable({ pattern }) {
  ...
  const { botExceptions, getBotExceptions, updateBotException, saveBotExceptionsOrder, setErr } = useBMConfig();
  ...
  const [botExceptionID, setBotExceptionID] = React.useState();
  const [botExceptionName, setBotExceptionName] = React.useState([]);
  const prepareAction = () => {
    if (selected.length > 1) {
      let tmpName = [];
      botExceptions.forEach((botException) => {
        if (selected.indexOf(botException.id) !== -1) tmpName.push({ id: botException.id, name: botException.name });
      });
      setBotExceptionName(tmpName);
    } else {
      botExceptions.forEach((botException) => {
        if (botException.id === botExceptionID) setBotExceptionName([{ id: botException.id, name: botException.name }]);
      });
    }
  };
  ...
  React.useEffect(() => {
    if (botExceptions !== null) {
      setRows(botExceptions);
    } else {
      setRows(null);
    }
    setOrderChanged(false);
  ...
  const editClick = () => {
    // if (UserRole.READONLY_USER === userRole) return;
    navigate(`/application/${siteUid}/bot/exception/edit/${botExceptionID}`);
  };
  const changeBotExceptionEnable = (event, id) => {
    const value = { enabled: !rows[event.target.name].enabled };
    const list = [...rows];
    list[event.target.name]["enabled"] = !rows[event.target.name].enabled;
    setRows(list);
    updateBotException(siteUid, id, value);
  };
  const onClickSaveBotExceptionsOrder = async (event) => {
    const value = { bot_exception_ids: rows.map((_row) => _row.id) };
    setErr(null);
    await saveBotExceptionsOrder(siteUid, value);
    setOrderChanged(false);
  };
  ...
  return (
    <div>
        ...
        <TableContainer>
          <Table aria-labelledby="tableTitle" size={"medium"} aria-label="enhanced table">
            <EnhancedTableHead
              ...
              onSaveRateLimitRulesOrderClick={onClickSaveBotExceptionsOrder}
              ...
              <>
                {rows?.length === 0 ? (
                  <TableBody>
                    <TableRow>
                      <TableCell colSpan={9}>
                        <Typography textAlign="center">There are no bot exceptions.</Typography>
                      </TableCell>
                    </TableRow>
                  </TableBody>
                ) : (
                        ...
                        >
                          ...
                          <TableCell align="left" sx={{ padding: "8px" }}>
                            <IOSSwitch
                              ...
                              onChange={(event) => changeBotExceptionEnable(event, row?.id)}
                            />
                          </TableCell>
                          <TableCell align="left" sx={{ padding: "8px" }}>
      ...
      <DeleteBotExceptionModal
        open={deleteOpen}
        handleClose={deleteHandleClose}
        botExceptionID={0 < selected.length ? selected : botExceptionID}
        botExceptionName={botExceptionName}
...
function BotExceptionTable({ pattern }) {
  return <EnhancedTable pattern={pattern} />;
}
export default BotExceptionTable;
