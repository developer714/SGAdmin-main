...
function SplunkModal({ open, handleClose, data, wait, enable }) {
  ...
  const save = async () => {
    ...
    const result = await saveWebhookInfo(ExternalLogType.SPLUNK, { enabled: enableLog, sites: tmp, url, token, index });
    if (result) {
      setSuccess("success");
      setMessage("Success");
      setSnackOpen(true);
  ...
  const test = async () => {
    setConnecting(true);
    const result = await connectionTest(ExternalLogType.SPLUNK, { url, token, index });
    setConnecting(false);
    if (result) {
      setSuccess("success");
      setMessage(result);
      ...
      >
        <Box sx={{ width: "655px" }}>
          <Grid container px={3.5} py={1.5} borderBottom={"solid 1px #ccc"} alignItems="center" justifyContent="space-between">
            <Typography variant="h2">Splunk Webhook Configuration</Typography>
            ...
            <>
              <Grid container p={3.5} borderBottom={"solid 1px #ccc"}>
                ...
                <Grid item xs={12} px={1.5} mt={4}>
                  ...
                  <TextField fullWidth color="secondary" value={token} onChange={(e) => changeToken(e)} />
                </Grid>
                <Grid item xs={12} px={1.5} mt={4}>
                  <Typography variant="h3" mb={1.5}>
                    Index
