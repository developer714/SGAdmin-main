...
function SumoLogicModal({ open, handleClose, data, wait, enable }) {
  ...
  const save = async () => {
    ...
    const result = await saveWebhookInfo(ExternalLogType.SUMO_LOGIC, { enabled: enableLog, sites: tmp, url });
    if (result) {
      setSuccess("success");
      setMessage("Success");
      setSnackOpen(true);
  ...
  const test = async () => {
    setConnecting(true);
    const result = await connectionTest(ExternalLogType.SUMO_LOGIC, { url });
    setConnecting(false);
    if (result) {
      setSuccess("success");
      setMessage(result);
      ...
      >
        <Box sx={{ width: "655px" }}>
          <Grid container px={3.5} py={1.5} borderBottom={"solid 1px #ccc"} alignItems="center" justifyContent="space-between">
            <Typography variant="h2">Sumologic Webhook Configuration</Typography>
