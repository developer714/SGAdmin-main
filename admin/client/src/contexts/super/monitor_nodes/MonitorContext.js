...
function MonitorProvider({ children }) {
  ...
  const getServerMonitor = useCallback(async () => {
    const signal = checkSignal();
    try {
      const response = await axios.get("health/server", { signal });
      dispatch({
        type: GET_SERVER_MONITOR,
        payload: {
  ...
  const getWafMonitor = useCallback(
    async (wafID) => {
      ...
      try {
        const response = await axios.get(`health/waf/${wafID}`, {
          signal,
        });
        dispatch({
  ...
  const getWAFEdges = useCallback(async () => {
    try {
      const response = await axios.get("waf/point");
      dispatch({
        type: GET_WAFEDGES,
        payload: {
          edges: response.data,
