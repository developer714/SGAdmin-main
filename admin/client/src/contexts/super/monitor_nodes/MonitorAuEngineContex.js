...
const MonitorAuEngineReducer = (state, action) => {
  switch (action.type) {
...
const MonitorAuEngineContext = createContext(null);
function MonitorAuEngineProvider({ children }) {
  const [state, dispatch] = useReducer(MonitorAuEngineReducer, initialState);
  ...
  const getWafMonitor = useCallback(
    async (wafID) => {
      ...
      try {
        const response = await axios.get(`health/au_engine/${wafID}`, {
          signal,
        });
        dispatch({
  ...
  const getWAFEdges = useCallback(async () => {
    try {
      const response = await axios.get("au_engine/point");
      dispatch({
        type: GET_WAFEDGES,
        payload: {
          edges: response.data,
  ...
  return (
    <MonitorAuEngineContext.Provider
      value={{
        ...state,
        getWafMonitor,
...
export { MonitorAuEngineContext, MonitorAuEngineProvider };
