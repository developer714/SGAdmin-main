...
const MonitorAdEngineReducer = (state, action) => {
  switch (action.type) {
...
const MonitorAdEngineContext = createContext(null);
function MonitorAdEngineProvider({ children }) {
  const [state, dispatch] = useReducer(MonitorAdEngineReducer, initialState);
  ...
  const getWafMonitor = useCallback(
    async (wafID) => {
      ...
      try {
        const response = await axios.get(`health/ad_engine/${wafID}`, {
          signal,
        });
        dispatch({
  ...
  const getWAFEdges = useCallback(async () => {
    try {
      const response = await axios.get("ad_engine/point");
      dispatch({
        type: GET_WAFEDGES,
        payload: {
          edges: response.data,
  ...
  return (
    <MonitorAdEngineContext.Provider
      value={{
        ...state,
        getWafMonitor,
...
export { MonitorAdEngineContext, MonitorAdEngineProvider };
