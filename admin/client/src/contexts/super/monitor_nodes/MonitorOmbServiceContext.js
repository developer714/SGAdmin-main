...
const MonitorOmbServiceReducer = (state, action) => {
  switch (action.type) {
...
const MonitorOmbServiceContext = createContext(null);
function MonitorOmbServiceProvider({ children }) {
  const [state, dispatch] = useReducer(MonitorOmbServiceReducer, initialState);
  ...
  const getWafMonitor = useCallback(
    async (wafID) => {
      ...
      try {
        const response = await axios.get(`health/omb_service/${wafID}`, {
          signal,
        });
        dispatch({
    ...
    async (wafID) => {
      ...
      try {
        const response = await axios.post(
          `health/omb_service/stats/${wafID}`,
          {
  ...
  const getWAFEdges = useCallback(async () => {
    try {
      const response = await axios.get("omb_service/point");
      dispatch({
        type: GET_WAFEDGES,
        payload: {
          edges: response.data,
  ...
  return (
    <MonitorOmbServiceContext.Provider
      value={{
        ...state,
        getWafMonitor,
...
export { MonitorOmbServiceContext, MonitorOmbServiceProvider };
