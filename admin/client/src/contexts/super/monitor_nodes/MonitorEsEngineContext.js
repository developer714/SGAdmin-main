...
const MonitorEsEngineReducer = (state, action) => {
  switch (action.type) {
...
const MonitorEsEngineContext = createContext(null);
function MonitorEsEngineProvider({ children }) {
  const [state, dispatch] = useReducer(MonitorEsEngineReducer, initialState);
  ...
  const getWafMonitor = useCallback(
    async (wafID) => {
      ...
      try {
        const response = await axios.get(`health/es_engine/${wafID}`, {
          signal,
        });
        dispatch({
  ...
  const getWAFEdges = useCallback(async () => {
    try {
      const response = await axios.get("es_engine/point");
      dispatch({
        type: GET_WAFEDGES,
        payload: {
          edges: response.data,
  ...
  return (
    <MonitorEsEngineContext.Provider
      value={{
        ...state,
        getWafMonitor,
...
export { MonitorEsEngineContext, MonitorEsEngineProvider };
