...
const MonitorBmEngineReducer = (state, action) => {
  switch (action.type) {
...
const MonitorBmEngineContext = createContext(null);
function MonitorBmEngineProvider({ children }) {
  const [state, dispatch] = useReducer(MonitorBmEngineReducer, initialState);
  ...
  const getWafMonitor = useCallback(
    async (wafID) => {
      ...
      try {
        const response = await axios.get(`health/bm_engine/${wafID}`, {
          signal,
        });
        dispatch({
  ...
  const getWAFEdges = useCallback(async () => {
    try {
      const response = await axios.get("bm_engine/point");
      dispatch({
        type: GET_WAFEDGES,
        payload: {
          edges: response.data,
  ...
  return (
    <MonitorBmEngineContext.Provider
      value={{
        ...state,
        getWafMonitor,
...
export { MonitorBmEngineContext, MonitorBmEngineProvider };
