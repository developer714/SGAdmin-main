...
const GET_ES_ENGINES = "GET_ES_ENGINES";
...
const EsEngineContext = createContext(null);

function EsEngineProvider({ children }) {
  ...
  const getWAF = useCallback(
    async (size, from, init = true) => {
      ...
      try {
        const response = await axios.post("es_engine/point", {
          from,
          size,
        });
        dispatch({
  ...
  const createWAF = useCallback(
    async (values) => {
      try {
        await axios.put("es_engine/point", values);
        getWAF(state.size, 0);
      } catch (err) {
        setErr(err.message);
      }
  ...
  const updateWAF = useCallback(
    async (wafID, values) => {
      try {
        await axios.post(`es_engine/point/${wafID}`, values);
        getWAF(state.size, 0);
      } catch (err) {
        setErr(err.message);
      }
  ...
  const deleteWAF = useCallback(
    async (wafID, remove) => {
      try {
        await axios.delete(`es_engine/point/${wafID}`, {
          data: { remove },
        });
        getWAF(state.size, 0);
      } catch (err) {
  ...
  const restoreWAF = useCallback(
    async (wafID) => {
      try {
        await axios.patch(`es_engine/point/${wafID}`);
        getWAF(state.size, 0);
      } catch (err) {
        setErr(err.message);
      }
  ...
  const viewWAF = useCallback(
    async (wafID) => {
      try {
        const response = await axios.get(`es_engine/point/${wafID}`);
...
export { EsEngineContext, EsEngineProvider };
