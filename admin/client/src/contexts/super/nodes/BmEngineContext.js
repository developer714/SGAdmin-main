...
const GET_BM_ENGINES = "GET_BM_ENGINES";
...
const BmEngineContext = createContext(null);

function BmEngineProvider({ children }) {
  ...
  const getWAF = useCallback(
    async (size, from, init = true) => {
      ...
      try {
        const response = await axios.post("bm_engine/point", {
          from,
          size,
        });
        dispatch({
  ...
  const createWAF = useCallback(
    async (values) => {
      try {
        await axios.put("bm_engine/point", values);
        getWAF(state.size, 0);
      } catch (err) {
        setErr(err.message);
      }
  ...
  const updateWAF = useCallback(
    async (wafID, values) => {
      try {
        await axios.post(`bm_engine/point/${wafID}`, values);
        getWAF(state.size, 0);
      } catch (err) {
        setErr(err.message);
      }
  ...
  const deleteWAF = useCallback(
    async (wafID, remove) => {
      try {
        await axios.delete(`bm_engine/point/${wafID}`, {
          data: { remove },
        });
        getWAF(state.size, 0);
      } catch (err) {
  ...
  const restoreWAF = useCallback(
    async (wafID) => {
      try {
        await axios.patch(`bm_engine/point/${wafID}`);
        getWAF(state.size, 0);
      } catch (err) {
        setErr(err.message);
      }
  ...
  const viewWAF = useCallback(
    async (wafID) => {
      try {
        const response = await axios.get(`bm_engine/point/${wafID}`);
...
export { BmEngineContext, BmEngineProvider };
